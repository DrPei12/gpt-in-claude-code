...
mkdir -p "$tmp/home/.config/gpt-in-claude-code" "$tmp/home/.cli-proxy-api" "$tmp/home/.codex" "$tmp/bin"
: > "$tmp/home/.config/gpt-in-claude-code/cliproxyapi.yaml"
printf '%s\n' \
  'GICC_PROXY_TOKEN=test-token' \
  "GICC_CODEX_AUTH_DIR=$tmp/home/.cli-proxy-api" \
  "GICC_PROXY_CONFIG=$tmp/home/.config/gpt-in-claude-code/cliproxyapi.yaml" \
  > "$tmp/home/.config/gpt-in-claude-code/env"
cp "$root/settings.json" "$tmp/home/.config/gpt-in-claude-code/settings.json"
cp "$root/usage-limit" "$tmp/home/.config/gpt-in-claude-code/usage-limit"
cp "$root/codex-session" "$tmp/home/.config/gpt-in-claude-code/codex-session"
cp "$root/preload.cjs" "$tmp/home/.config/gpt-in-claude-code/preload.cjs"
cp "$root/skill-bridge.cjs" "$tmp/home/.config/gpt-in-claude-code/skill-bridge.cjs"
...
chmod +x "$tmp/home/.config/gpt-in-claude-code/codex-session"

# Values sourced from GICC's Unix env file must reach the Node skill bridge.
skill_env_home="$tmp/skill-env-home"
skill_env_claude="$tmp/custom-claude-home"
mkdir -p "$skill_env_home/.config/gpt-in-claude-code" "$skill_env_home/.agents/skills/env-codex" "$skill_env_claude/skills/env-claude"
cp "$root/skill-bridge.cjs" "$skill_env_home/.config/gpt-in-claude-code/skill-bridge.cjs"
printf '%s\n' \
  'GICC_SKILL_BRIDGE=on' \
  'GICC_SKILL_PLUGINS=off' \
  'GICC_SKILL_DOLLAR_REFERENCES=off' \
  "GICC_CLAUDE_CONFIG_DIR=$skill_env_claude" \
  "GICC_CODEX_ADMIN_SKILLS_DIR=$tmp/missing-admin-skills" \
  > "$skill_env_home/.config/gpt-in-claude-code/env"
...
skill_env_output=$(HOME="$skill_env_home" PATH="$tmp/bin:$PATH" "$root/gicc" skills)
...
if [[ "${1:-}" == "update" ]]; then
  ...
  [[ -z "${FAKE_UPDATE_ENV_LOG:-}" ]] || {
    printf 'PROXY_TOKEN=%s\n' "${GICC_PROXY_TOKEN:-}" >> "$FAKE_UPDATE_ENV_LOG"
    printf 'AUTH_TOKEN=%s\n' "${ANTHROPIC_AUTH_TOKEN:-}" >> "$FAKE_UPDATE_ENV_LOG"
    printf 'MANAGED=%s\n' "${GICC_MANAGED_SESSION:-}" >> "$FAKE_UPDATE_ENV_LOG"
...
if [[ -n "${FAKE_FABLEPLAN_TERRA_PROMPT_FILE:-}" ]]; then
  ...
  for (( argument_index = 0; argument_index < ${#arguments[@]}; argument_index++ )); do
    if [[ "${arguments[$argument_index]}" == --add-dir && "${arguments[$((argument_index + 1))]:-}" == *gicc-fableplan.* ]]; then
...
printf '%s\n' "NO_SESSION_PERSISTENCE=${GICC_NO_SESSION_PERSISTENCE:-}"
...
printf '%s\n' "PROXY_TOKEN_SET=${GICC_PROXY_TOKEN:+yes}"
...
if [[ "${1:-}" == native-test ]]; then
  ...
  printf '%s\n' "NATIVE_CODEX_PROXY_TOKEN_SET=${GICC_PROXY_TOKEN:+yes}"
  exit 0
fi
[[ "${1:-}" == app-server ]] || exit 2
...
cat > "$tmp/bin/ps" <<'EOF'
#!/usr/bin/env bash
[[ "${GICC_TEST_PS_FAIL:-0}" != 1 ]] || exit 1
printf '%s\n' "${GICC_TEST_PROCESS_IDENTITY:-test-process-identity}"
EOF
chmod +x "$tmp/bin/"*
...
fs.writeFileSync(process.env.GICC_TEST_WRAPPER_PID_FILE, `${child.pid}\n`);
...
native_codex_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_NODE_BIN=/missing/node \
  GICC_PROXY_TOKEN=must-not-leak CODEX_HOME=/native/codex/home \
  CLAUDE_CONFIG_DIR=/native/codex/profile "$root/gicc" codex native-test 'arg with spaces')
...
managed_preload="--preload $tmp/home/.config/gpt-in-claude-code/preload.cjs"
native_claude_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_NODE_BIN=/missing/node \
  GICC_CLAUDE_CONFIG_DIR="$native_claude_profile" CLAUDE_CONFIG_DIR=/managed/profile \
  ...
  GICC_INTERACTIVE_TUI=1 GICC_MANAGED_SESSION=1 BUN_OPTIONS="$managed_preload --preload /user/preload.cjs" \
  "$root/gicc" claude native-test 'arg with spaces')
...
native_user_claude_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_NODE_BIN=/missing/node \
  CLAUDE_CONFIG_DIR=/user/claude-profile ANTHROPIC_BASE_URL=https://custom-provider.invalid \
  ANTHROPIC_AUTH_TOKEN=native-provider-token GICC_PROXY_TOKEN=must-not-leak \
  ...
  "$root/gicc" claude native-user-settings)
...
mkdir -p "$native_broken_env_home/.config/gpt-in-claude-code"
printf '%s\n' 'return 77' > "$native_broken_env_home/.config/gpt-in-claude-code/env"
native_broken_env=$(HOME="$native_broken_env_home" PATH="$tmp/bin:$PATH" \
  "$root/gicc" claude native-broken-env)
...
for native_selector in fable opus sonnet haiku; do
  native_selector_output=$(HOME="$native_broken_env_home" PATH="$tmp/bin:$PATH" \
    "$root/gicc" "--$native_selector" 'prompt with spaces' --permission-mode plan)
...
native_full_model_output=$(HOME="$native_broken_env_home" PATH="$tmp/bin:$PATH" \
  "$root/gicc" --claude-model "$native_full_model" 'literal;not-shell')
...
if native_model_error=$(HOME="$native_broken_env_home" PATH="$tmp/bin:$PATH" \
    "$root/gicc" --claude-model 2>&1); then
...
if HOME="$tmp/home" PATH="$tmp/bin:$PATH" FAKE_CLAUDE_EXIT=37 \
    "$root/gicc" claude native-exit >/dev/null 2>&1; then
...
hosted_remote_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" \
  ...
  GICC_PROXY_URL=http://127.0.0.1:9999 GICC_CHATGPT_PLAN_LABEL=ChatGPT-Proxied \
  "$root/gicc" --remote-control=pairing-name)
...
hosted_rc_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" "$root/gicc" --rc pair-name)
[[ "$hosted_rc_output" == *'ARGC=2'* && "$hosted_rc_output" == *'ARGS=--rc pair-name'* ]]
hosted_review_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" "$root/gicc" ultrareview --help)
[[ "$hosted_review_output" == *'ARGS=ultrareview --help'* ]]
positional_remote_output=$(HOME="$native_broken_env_home" PATH="$tmp/bin:$PATH" \
  ANTHROPIC_API_KEY=positional-api-key "$root/gicc" remote-control)
...
debug_hosted_output=$(HOME="$native_broken_env_home" PATH="$tmp/bin:$PATH" \
  "$root/gicc" -d hosted --remote-control=debug-session)
...
trailing_version_output=$(HOME="$missing_maintenance_home" PATH="$tmp/bin:$PATH" \
  "$root/gicc" literal-prompt --version)
...
if HOME="$tmp/home" PATH="$tmp/bin:$PATH" FAKE_CLAUDE_EXIT=41 \
    "$root/gicc" --remote-control >/dev/null 2>&1; then
...
#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' "${1:-missing}" >> "$GICC_TEST_AUTH_RECOVERY_LOG"
case "${1:-}" in
  sync)
    if [[ ! -e "$GICC_TEST_AUTH_RECOVERY_MARKER" ]]; then
      : > "$GICC_TEST_AUTH_RECOVERY_MARKER"
      exit 11
    fi
    ;;
  login|watch|status) ;;
...
node --check "$root/bin/gicc-package.mjs"
...
GICC_NODE_BIN="$configured_node_bin" PATH="$old_node_bin:$PATH" run_wrapper skills \
  >"$tmp/configured-node.stdout" 2>"$tmp/configured-node.stderr"
[[ "$(<"$tmp/configured-node.stderr")" == *'gicc skills:'* || ! -s "$tmp/configured-node.stderr" ]]
if GICC_NODE_BIN=relative/node run_wrapper skills >"$tmp/relative-node.stdout" 2>"$tmp/relative-node.stderr"; then
  printf '%s\n' 'expected a relative GICC_NODE_BIN to be rejected' >&2
  exit 1
fi
grep -F 'GICC_NODE_BIN must be an absolute directory' "$tmp/relative-node.stderr" >/dev/null
...
interactive_auth_output=$(
  HOME="$tmp/home" PATH="$tmp/bin:$PATH" CI=0 GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=1 \
    GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 \
    GICC_CODEX_SESSION_HELPER="$auth_recovery_helper" \
    GICC_TEST_AUTH_RECOVERY_LOG="$auth_recovery_log" GICC_TEST_AUTH_RECOVERY_MARKER="$auth_recovery_marker" \
    GICC_TEST_TTY_INPUT=1 GICC_TEST_TTY_OUTPUT=1 \
    "$root/gicc" --terra auth-recovery-test 2>&1
)
...
[[ "$noninteractive_auth_output" == *'Run `gicc --login` in an interactive terminal'* ]]
...
if ci_auth_output=$(
  HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=1 \
    GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 CI=1 \
    GICC_CODEX_SESSION_HELPER="$auth_recovery_helper" \
    GICC_TEST_AUTH_RECOVERY_LOG="$auth_recovery_log" GICC_TEST_AUTH_RECOVERY_MARKER="$auth_recovery_marker" \
    GICC_TEST_TTY_INPUT=1 GICC_TEST_TTY_OUTPUT=1 \
    "$root/gicc" --terra auth-recovery-test 2>&1
...
! grep -q '^login$' "$auth_recovery_log"
input_alias_output=$(GICC_TEST_TTY_INPUT=1 node -e '
...
input_listener_output=$(printf '/model solplan\r' | GICC_TEST_TTY_INPUT=1 node --require "$root/preload.cjs" -e '
...
jq -e '
  ...
  and .statusLine.command == "__GICC_STATUSLINE_COMMAND__"
  and (.env | not)
' "$root/settings.json" >/dev/null

legacy_auto_config="$tmp/legacy-auto-config"
mkdir -p "$legacy_auto_config"
cp "$tmp/home/.config/gpt-in-claude-code/env" "$legacy_auto_config/env"
jq '.autoMode = {
...
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CONFIG_DIR="$legacy_auto_config" \
  GICC_SKIP_AUTO_UPDATE=1 FAKE_AUTO_MODE_DEFAULTS_FAIL=1 \
  "$root/gicc" auto-mode config >/dev/null
...
cp "$tmp/home/.config/gpt-in-claude-code/env" "$custom_fallback_config/env"
jq '.autoMode = {allow: ["User custom allow rule"]}' \
  "$root/settings.json" > "$custom_fallback_config/settings.json"
if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CONFIG_DIR="$custom_fallback_config" \
    GICC_SKIP_AUTO_UPDATE=1 FAKE_AUTO_MODE_DEFAULTS_FAIL=1 \
    "$root/gicc" auto-mode config >/dev/null 2>&1; then
...
jq '.autoMode = {
  ...
  "$tmp/home/.config/gpt-in-claude-code/settings.json" > "$tmp/custom-auto-mode-settings.json"
mv "$tmp/custom-auto-mode-settings.json" "$tmp/home/.config/gpt-in-claude-code/settings.json"
default_output=$(run_wrapper --terra test-prompt)
state_file="$tmp/home/.config/gpt-in-claude-code/.claude.json"
jq -e '
...
[[ "$default_output" == *"BUN=--preload $tmp/home/.config/gpt-in-claude-code/preload.cjs"* ]]
...
instruction_bridge_off=$(GICC_INSTRUCTION_BRIDGE=off run_wrapper instruction-bridge-off)
[[ "$instruction_bridge_off" == *'INSTRUCTION_BRIDGE=off'* ]]
if GICC_INSTRUCTION_BRIDGE=invalid run_wrapper instruction-bridge-invalid >/dev/null 2>&1; then
  printf '%s\n' 'expected an invalid instruction bridge mode to fail' >&2
  exit 1
fi

if GICC_PROXY_URL=https://proxy.example.test run_wrapper remote-proxy-test \
...
remote_proxy_output=$(GICC_PROXY_URL=https://proxy.example.test GICC_ALLOW_REMOTE_PROXY=1 \
  run_wrapper remote-proxy-test)
[[ "$remote_proxy_output" == *'BASE=https://proxy.example.test'* ]]
if GICC_PROXY_URL=http://proxy.example.test GICC_ALLOW_REMOTE_PROXY=1 run_wrapper remote-proxy-test \
    >"$tmp/insecure-remote.stdout" 2>"$tmp/insecure-remote.stderr"; then
  printf '%s\n' 'expected an opted-in remote proxy to require HTTPS' >&2
  exit 1
fi

interactive_wrapper_output=$(GICC_TEST_TTY_OUTPUT=1 run_wrapper --terra interactive-render-test)
[[ "$interactive_wrapper_output" == *'INTERACTIVE=1'* ]]
[[ "$interactive_wrapper_output" == *'CHATGPT_PLAN=ChatGPT Pro'* ]]
printf '%s\n' '{malformed' > "$tmp/home/.config/gpt-in-claude-code/usage-cache/limits.json"
repaired_plan_output=$(GICC_TEST_TTY_OUTPUT=1 run_wrapper --terra repaired-plan-cache-test)
[[ "$repaired_plan_output" == *'CHATGPT_PLAN=ChatGPT Pro'* ]]
jq -e '.plan_type == "pro"' "$tmp/home/.config/gpt-in-claude-code/usage-cache/limits.json" >/dev/null
...
EOF
rm -f "$tmp/home/.config/gpt-in-claude-code/run/skill-warning-state"
CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=inherited GICC_SKILL_BRIDGE_HELPER="$warning_bridge" run_wrapper warning-test \
...
GICC_SKILL_BRIDGE_HELPER="$warning_bridge" run_wrapper warning-test \
...
jq -e '
  ...
  and (.autoMode.hard_deny | any(startswith("Data Exfiltration:")
    and contains("GICC scoped private development transfer exception:")
    and contains("public destination") and contains("credentials or secrets") and contains("different host")))
' "$tmp/home/.config/gpt-in-claude-code/settings.json" >/dev/null
...
jq -e '
  ...
  and ([.autoMode.soft_deny[] | select(. == "User custom soft deny rule")] | length == 1)
  and (.autoMode.hard_deny | any(startswith("Data Exfiltration: updated hard deny") and contains("GICC scoped private development transfer exception:")))
  and ([.autoMode.hard_deny[] | select(. == "User custom hard deny rule")] | length == 1)
' "$tmp/home/.config/gpt-in-claude-code/settings.json" >/dev/null

proxy_lock="$tmp/home/.config/gpt-in-claude-code/run/proxy-start.lock"
stop_fake_managed_proxy() {
  local metadata="$tmp/home/.config/gpt-in-claude-code/run/managed-proxy" pid=""
  if [[ -r "$metadata" ]]; then
...
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 \
  GICC_TEST_MODE=1 GICC_TEST_SKIP_AUTH_SYNC=1 GICC_SKILL_BRIDGE=off GICC_TEST_FORCE_HARDLINK_FAILURE=1 \
  GICC_TEST_PROXY_REACHABLE_FILE="$proxy_fallback_ready" \
  FAKE_PROXY_READY_FILE="$proxy_fallback_ready" FAKE_PROXY_START_LOG="$proxy_fallback_log" \
  "$root/gicc" proxy-fallback-test >/dev/null
...
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 \
  GICC_TEST_MODE=1 GICC_TEST_SKIP_AUTH_SYNC=1 GICC_SKILL_BRIDGE=off GICC_TEST_FORCE_PUBLICATION_FAILURE=1 \
  GICC_TEST_FORCE_PUBLICATION_FAILURE_MATCH=proxy-start.lock \
  GICC_TEST_PROXY_LOCK_ATTEMPT_FILE="$proxy_partial_attempt" \
  FAKE_PROXY_READY_FILE="$tmp/proxy-partial-ready" FAKE_PROXY_NEVER_READY=1 \
  "$root/gicc" proxy-partial-test >"$tmp/proxy-partial.out" 2>"$tmp/proxy-partial.err" &
proxy_partial_pid=$!
...
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 \
  GICC_TEST_MODE=1 GICC_TEST_SKIP_AUTH_SYNC=1 GICC_SKILL_BRIDGE=off GICC_TEST_PROXY_LOCK_ATTEMPT_FILE="$proxy_ownerless_attempt" \
  FAKE_PROXY_READY_FILE="$tmp/proxy-ownerless-ready" FAKE_PROXY_NEVER_READY=1 \
  "$root/gicc" proxy-ownerless-test >"$tmp/proxy-ownerless.out" 2>"$tmp/proxy-ownerless.err" &
proxy_ownerless_pid=$!
...
proxy_race_base=(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 \
  GICC_TEST_MODE=1 GICC_TEST_SKIP_AUTH_SYNC=1 GICC_SKILL_BRIDGE=off GICC_TEST_PROCESS_IDENTITY=test-process-identity \
  GICC_TEST_LOCK_MATCH=proxy-start.lock GICC_TEST_PROXY_REACHABLE_FILE="$proxy_race_ready" \
  FAKE_PROXY_READY_FILE="$proxy_race_ready" FAKE_PROXY_START_LOG="$proxy_race_log")
env "${proxy_race_base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/proxy-a-mkdir" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/proxy-a-continue" \
  GICC_TEST_PROXY_LOCK_ATTEMPT_FILE="$tmp/proxy-a-attempt" \
  "$root/gicc" proxy-a-test >/dev/null &
proxy_a=$!
for _ in {1..1000}; do [[ -e "$tmp/proxy-a-mkdir" ]] && break; sleep 0.02; done
[[ -e "$tmp/proxy-a-mkdir" ]]
touch -t 200001010000 "$proxy_lock"
env "${proxy_race_base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/proxy-b-publish" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/proxy-b-continue" \
  "$root/gicc" proxy-b-test >/dev/null &
proxy_b=$!
...
printf 'pid=99999999\nidentity=dead\nnonce=proxy-x\n' > "$proxy_lock/owner"
touch -t 200001010000 "$proxy_lock"
env "${proxy_race_base[@]}" GICC_TEST_LOCK_BEFORE_RENAME_READY="$tmp/proxy-x-before" \
  GICC_TEST_LOCK_BEFORE_RENAME_CONTINUE="$tmp/proxy-x-before-continue" \
  GICC_TEST_LOCK_AFTER_RENAME_READY="$tmp/proxy-x-after" \
  GICC_TEST_LOCK_AFTER_RENAME_CONTINUE="$tmp/proxy-x-after-continue" \
  "$root/gicc" proxy-x-test >/dev/null &
proxy_x=$!
for _ in {1..1000}; do [[ -e "$tmp/proxy-x-before" ]] && break; sleep 0.02; done
[[ -e "$tmp/proxy-x-before" ]]
env "${proxy_race_base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/proxy-y-publish" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/proxy-y-continue" \
  GICC_TEST_LOCK_SELF_RECOVERED_FILE="$tmp/proxy-y-recovered" \
  "$root/gicc" proxy-y-test >/dev/null &
proxy_y=$!
...
[[ -e "$tmp/proxy-x-after" ]]
env "${proxy_race_base[@]}" GICC_TEST_PROXY_LOCK_ATTEMPT_FILE="$tmp/proxy-z-attempt" \
  "$root/gicc" proxy-z-test >/dev/null &
proxy_z=$!
...
env "${proxy_race_base[@]}" GICC_TEST_PROXY_LOCK_ATTEMPT_FILE="$proxy_release_attempt" \
  FAKE_PROXY_NEVER_READY=1 "$root/gicc" proxy-release-test >/dev/null 2>"$tmp/proxy-release.err" &
proxy_release_pid=$!
for _ in {1..1000}; do
...
touch -t 200001010000 "$proxy_lock"
env "${proxy_race_base[@]}" "$root/gicc" proxy-reused-pid-test >/dev/null
...
mkdir -p "$proxy_lock"
touch -t 200001010000 "$proxy_lock"
env "${proxy_race_base[@]}" "$root/gicc" proxy-stale-ownerless-test >/dev/null
[[ -e "$proxy_race_ready" && -s "$proxy_race_log" && ! -e "$proxy_lock" ]]
stop_fake_managed_proxy
...
: > "$proxy_ready_file"
proxy_recovery_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=0 \
  GICC_TEST_MODE=1 GICC_TEST_SKIP_AUTH_SYNC=1 GICC_SKILL_BRIDGE=off GICC_TEST_PROCESS_IDENTITY=test-process-identity \
  GICC_TEST_PROXY_REACHABLE_FILE="$proxy_ready_file" \
  ANTHROPIC_BASE_URL=https://caller.invalid ANTHROPIC_AUTH_TOKEN=caller-anthropic-secret \
  ANTHROPIC_API_KEY=caller-api-secret GICC_PROXY_TOKEN=test-token \
  ...
  FAKE_PROXY_RECOVERY=1 "$root/gicc" recovery-test)
[[ "$proxy_recovery_output" == *'PROXY_RECOVERED=1'* ]]
[[ "$(wc -l < "$proxy_start_log" | tr -d ' ')" == 1 ]]
[[ "$(<"$proxy_env_log")" == $'BASE_URL=\nAUTH_TOKEN=\nAPI_KEY=\nPROXY_TOKEN=\nCLAUDE_CONFIG=\nAUTO_MODEL=\nBG_MODEL=\nSUBAGENT_MODEL=\nBUN_OPTIONS=' ]]
[[ ! -e "$tmp/home/.config/gpt-in-claude-code/run/proxy-start.lock" ]]
...
live_owner_lock="$tmp/home/.config/gpt-in-claude-code/run/proxy-start.lock"
...
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  ...
  "$root/gicc" live-owner-first >"$tmp/live-owner-first.out" 2>"$tmp/live-owner-first.err" &
...
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 \
  GICC_TEST_MODE=1 GICC_TEST_SKIP_AUTH_SYNC=1 GICC_SKILL_BRIDGE=off GICC_TEST_PROCESS_IDENTITY=test-process-identity \
  GICC_TEST_PROXY_LOCK_ATTEMPT_FILE="$tmp/live-owner-second-attempt" \
  FAKE_PROXY_READY_FILE="$live_owner_ready_file" FAKE_PROXY_START_LOG="$live_owner_start_log" FAKE_PROXY_NEVER_READY=1 \
  "$root/gicc" live-owner-second >"$tmp/live-owner-second.out" 2>"$tmp/live-owner-second.err" &
...
if [[ -r "$tmp/home/.config/gpt-in-claude-code/run/managed-proxy" ]]; then
  live_owner_proxy_pid=$(awk -F= '$1 == "pid" { print $2; exit }' "$tmp/home/.config/gpt-in-claude-code/run/managed-proxy")
  [[ -z "$live_owner_proxy_pid" ]] || kill -TERM "$live_owner_proxy_pid" 2>/dev/null || true
  rm -f "$tmp/home/.config/gpt-in-claude-code/run/managed-proxy"
fi
...
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=0 \
  GICC_TEST_MODE=1 GICC_TEST_PROCESS_IDENTITY=test-process-identity \
  GICC_TEST_PROXY_REACHABLE_FILE="$proxy_ready_file" \
  FAKE_PROXY_READY_FILE="$proxy_ready_file" FAKE_PROXY_START_LOG="$transient_proxy_start_log" \
  FAKE_PROXY_TRANSIENT_ONCE=1 "$root/gicc" transient-health-test >/dev/null
[[ ! -s "$transient_proxy_start_log" ]]
existing_managed_proxy_file="$tmp/home/.config/gpt-in-claude-code/run/managed-proxy"
...
EOF
auth_proxy_bin="$tmp/bin/gicc-proxy-auth-test"
...
managed_proxy_file="$tmp/home/.config/gpt-in-claude-code/run/managed-proxy"
mkdir -p "$(dirname "$managed_proxy_file")"

# A 401/403 is recoverable only when mode-restricted PID metadata proves that
# GICC owns the tracked process. The old process must be replaced exactly
...
if ! managed_401_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$proxy_auth_curl" \
  GICC_PROXY_BIN="$auth_proxy_bin" GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 \
  GICC_SKIP_PROXY_WATCHER=1 FAKE_PROXY_HEALTH_STATE="$proxy_auth_state" \
  FAKE_PROXY_AUTH_START_LOG="$proxy_auth_log" FAKE_PROXY_START_STATE=healthy \
  GICC_TEST_MODE=1 GICC_TEST_PROCESS_IDENTITY="$tracked_proxy_identity" \
  "$root/gicc" managed-401-test 2>"$tmp/managed-401.stderr"); then
...
printf '%s\n' unauthorized > "$proxy_auth_state"
: > "$proxy_auth_log"
if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$proxy_auth_curl" \
    GICC_PROXY_BIN="$auth_proxy_bin" GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_AUTH_WATCHER=1 \
    GICC_SKIP_PROXY_WATCHER=1 FAKE_PROXY_HEALTH_STATE="$proxy_auth_state" \
    FAKE_PROXY_AUTH_START_LOG="$proxy_auth_log" GICC_TEST_MODE=1 GICC_TEST_PROCESS_IDENTITY="$tracked_proxy_identity" \
    "$root/gicc" unmanaged-401-test \
    >"$tmp/unmanaged-401.stdout" 2>"$tmp/unmanaged-401.stderr"; then
  printf '%s\n' 'expected an unmanaged 401 listener to fail closed' >&2
  exit 1
fi
grep -F 'not proven GICC-managed' "$tmp/unmanaged-401.stderr" >/dev/null
...
if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$proxy_auth_curl" \
    ...
    GICC_TEST_MODE=1 GICC_TEST_PROCESS_IDENTITY="$tracked_proxy_identity" \
    "$root/gicc" never-ready-proxy-test >"$tmp/never-ready.stdout" 2>"$tmp/never-ready.stderr"; then
...
configured_model_output=$(GICC_MODEL=gpt-5.6-luna run_wrapper test-prompt)
[[ "$configured_model_output" == *'--model gpt-5.6-luna'* ]]
configured_model_override=$(GICC_MODEL=gpt-5.6-luna run_wrapper --terra test-prompt)
[[ "$configured_model_override" == *'--model gpt-5.6-terra'* ]]
if GICC_MODEL=claude-sonnet-5 run_wrapper test-prompt >/dev/null 2>&1; then
...
resume_footer_output=$(FAKE_CLAUDE_RESUME=1 GICC_TEST_TTY_OUTPUT=1 run_wrapper)
[[ "$resume_footer_output" != *$'\033[2A'* ]]
[[ "$resume_footer_output" == *$'Resume this session with GICC:\ngicc --resume 123e4567-e89b-12d3-a456-426614174000'* ]]
background_footer_output=$(FAKE_CLAUDE_RESUME=1 GICC_TEST_TTY_OUTPUT=1 \
  GICC_SKIP_AUTH_WATCHER=1 GICC_SKIP_PROXY_WATCHER=1 run_wrapper --bg)
[[ "$background_footer_output" != *'Resume this session with GICC:'* ]]

if interrupted_resume_output=$(FAKE_CLAUDE_RESUME=1 FAKE_CLAUDE_RESUME_EXIT=130 GICC_TEST_TTY_OUTPUT=1 run_wrapper); then
...
[[ "$interrupted_resume_output" == *$'Resume this session with GICC:\ngicc --resume 123e4567-e89b-12d3-a456-426614174000'* ]]
...
assert_launcher_signal_forwarding() {
  ...
  HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
    GICC_SKIP_AUTO_UPDATE=1 GICC_SKIP_PROXY_WATCHER=1 \
    GICC_TEST_WRAPPER_PID_FILE="$wrapper_pid_file" \
    ...
    node "$tmp/launcher-signal-driver.cjs" "$root/gicc" --claude-chrome --print signal-test &
  driver_pid=$!
...
prose_only_capabilities=$(FAKE_CLAUDE_HELP_PROSE_ONLY=1 GICC_SKILL_BRIDGE=off \
...
assert_worktree_bridge_mode() {
  : > "$worktree_bridge_log"
  GICC_SKILL_BRIDGE_HELPER="$worktree_bridge_probe" \
    GICC_TEST_WORKTREE_BRIDGE_LOG="$worktree_bridge_log" \
...
managed_maintenance_output=$(GICC_MANAGED_SESSION=1 GICC_PROXY_TOKEN=must-not-leak \
  ANTHROPIC_BASE_URL=https://managed.invalid ANTHROPIC_AUTH_TOKEN=managed-secret \
  BUN_OPTIONS="--preload $tmp/home/.config/gpt-in-claude-code/preload.cjs --preload /user/preload.cjs" \
...
prefixed_maintenance=$(GICC_NODE_BIN=relative/node run_wrapper --verbose mcp list)
...
chrome_configured_model=$(GICC_MODEL=gpt-5.6-luna run_wrapper --claude-chrome --print chrome-test)
...
managed_inherited_chrome=$(GICC_MANAGED_SESSION=1 GICC_PROXY_TOKEN=must-not-leak \
...
GICC_NODE_BIN=relative/node run_wrapper --version >/dev/null
GICC_NODE_BIN=relative/node run_wrapper update >/dev/null
if self_update_status=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_NODE_BIN=relative/node \
    GICC_SELF_UPDATE_HELPER="$root/self-update" "$root/gicc" self-update --status 2>&1); then
  self_update_status_code=0
else
  self_update_status_code=$?
fi
[[ "$self_update_status_code" == 1 ]]
[[ "$self_update_status" != *'GICC_NODE_BIN'* ]]
...
[[ "$doctor_output" == *'Terminal UI: fullscreen (launch command hidden while GICC is open)'* ]]
...
if HOME="$tmp/home" PATH="$tmp/bin:/opt/homebrew/bin:/usr/bin:/bin" GICC_CURL_BIN="$tmp/bin/curl" \
  "$root/gicc" --doctor >"$tmp/no-claude.out" 2>"$tmp/no-claude.err"; then
...
' "$tmp/home/.config/gpt-in-claude-code/usage-cache/limits.json" >/dev/null
if [[ "$(uname -s)" == Darwin ]]; then
  cache_mode=$(stat -f '%Lp' "$tmp/home/.config/gpt-in-claude-code/usage-cache/limits.json")
  cache_dir_mode=$(stat -f '%Lp' "$tmp/home/.config/gpt-in-claude-code/usage-cache")
else
  cache_mode=$(stat -c '%a' "$tmp/home/.config/gpt-in-claude-code/usage-cache/limits.json")
  cache_dir_mode=$(stat -c '%a' "$tmp/home/.config/gpt-in-claude-code/usage-cache")
fi
[[ "$cache_mode" == 600 ]]
[[ "$cache_dir_mode" == 700 ]]

fallback_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  FAKE_USAGE_FAIL=1 GICC_USAGE_SOURCE=web "$root/gicc" --usage-limit 2>&1)
[[ "$fallback_output" == *'live refresh failed; showing the last cached snapshot'* ]]
[[ "$fallback_output" == *'Codex 7-day: 18% remaining (82% used)'* ]]

appserver_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  FAKE_USAGE_FAIL=1 "$root/gicc" --usage-limit)
...
changed_schema_output=$(HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  FAKE_USAGE_CHANGED=1 "$root/gicc" --usage-limit)
...
[[ "$(<"$tmp/home/.config/gpt-in-claude-code/codex-usage-account")" == codex-test.json ]]
auto_account_output=$(run_wrapper --account auto)
[[ "$auto_account_output" == *'automatic'* ]]
[[ ! -e "$tmp/home/.config/gpt-in-claude-code/codex-usage-account" ]]
[[ ! -e "$tmp/home/.config/gpt-in-claude-code/usage-cache/summary" ]]
FAKE_USAGE_FAIL=1 run_wrapper --usage-limit >/dev/null

if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_PERMISSION_MODE=broken "$root/gicc" >/dev/null 2>&1; then
  printf '%s\n' 'expected invalid permission mode to fail' >&2
  exit 1
fi
if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_AUTO_MODE_MODEL=claude-sonnet-5 "$root/gicc" >/dev/null 2>&1; then
  printf '%s\n' 'expected Anthropic auto-mode classifier override to fail' >&2
  exit 1
fi

if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_AUTO_COMPACT_WINDOW=99999 "$root/gicc" >/dev/null 2>&1; then
  printf '%s\n' 'expected invalid auto-compact window to fail' >&2
  exit 1
fi

if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_MOUSE_POINTER_SHAPE=beam "$root/gicc" >/dev/null 2>&1; then
...
status_output=$(printf '%s\n' '{"session_id":"stable-session","model":{"id":"gpt-5.6-sol"},"effort":{"level":"xhigh"},"context_window":{"used_percentage":42.9,"total_input_tokens":171600,"context_window_size":400000}}' | \
  GICC_STATUSLINE_COLUMNS=200 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline" 2>"$tmp/statusline.stderr")
...
hostile_status=$(printf '%s\n' '{"session_id":"hostile-session","model":{"id":"hostile\u001b]0;MODEL-OSC\u0007\u009d0;MODEL-C1-OSC\u009c\u001b[31mMODEL-CSI\u001b[0m\u009b31mMODEL-C1\u061cMODEL-ALM\u200eMODEL-LRM\u200fMODEL-RLM\u202eMODEL-BIDI"},"effort":{"level":"high\u001b]8;;https://attacker.invalid\u0007max\u001b]8;;\u0007"},"context_window":{"used_percentage":5}}' | \
  GICC_USAGE_DISPLAY=off GICC_STATUSLINE_COLUMNS=200 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
[[ "$hostile_status" == $'\033[38;5;81mGICC\033[0m · \033[1mhostile\033[0m · high effort · 5% context' ]]
...
suffix_only_status=$(printf '%s\n' '{"session_id":"suffix-only-session","model":{"id":"\u001b]0;ignored\u0007gpt-5.6-sol","display_name":"safe fallback"},"effort":{"level":"\u001b]0;ignored\u0007max"},"context_window":{"used_percentage":7}}' | \
  GICC_USAGE_DISPLAY=off GICC_STATUSLINE_COLUMNS=200 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
[[ "$suffix_only_status" == $'\033[38;5;81mGICC\033[0m · \033[1msafe fallback\033[0m · adaptive effort · 7% context' ]]
...
printf '%s\n' $'safe summary \033]0;CACHE-OSC\007 \302\2350;CACHE-C1-OSC\302\234 \033[31mCACHE-CSI\033[0m \302\23331mCACHE-C1 \342\200\256CACHE-BIDI' \
  > "$tmp/home/.config/gpt-in-claude-code/usage-cache/summary"
date +%s > "$tmp/home/.config/gpt-in-claude-code/usage-cache/last-success"
hostile_cache_status=$(printf '%s\n' '{"session_id":"hostile-cache","model":{"id":"gpt-5.6-sol"},"context_window":{"used_percentage":5}}' | \
  GICC_STATUSLINE_COLUMNS=200 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
...
printf '%s\n' '{"session_id":"private-refresh","model":{"id":"gpt-5.6-sol"},"context_window":{"used_percentage":5}}' | \
  CLAUDE_CONFIG_DIR="$status_refresh_config" GICC_USAGE_LIMIT_BIN="$status_refresh_helper" \
...
narrow_status=$(printf '%s\n' '{"session_id":"narrow-session","model":{"id":"gpt-5.6-sol"},"effort":{"level":"xhigh"},"context_window":{"used_percentage":42.9}}' | \
  GICC_STATUSLINE_COLUMNS=40 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
...
locale_context_status=$(printf '%s\n' "$locale_parity_input" | LC_ALL=C \
  GICC_USAGE_DISPLAY=off GICC_STATUSLINE_COLUMNS=37 \
  CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
[[ "$locale_context_status" == $'\033[38;5;81mGICC\033[0m · \033[1mGPT-5.6 Terra\033[0m · 12% context' ]]
locale_tiny_status=$(printf '%s\n' "$locale_parity_input" | LC_ALL=C \
  GICC_USAGE_DISPLAY=off GICC_STATUSLINE_COLUMNS=12 \
  CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
[[ "$locale_tiny_status" == $'\033[38;5;81mGICC\033[0m · \033[1mGPT-…\033[0m' ]]

solplan_settings="$tmp/home/.config/gpt-in-claude-code/settings.json"
...
ultracode_status=$(printf '%s\n' '{"session_id":"ultracode-session","model":{"id":"gpt-5.6-sol"},"effort":{"level":"xhigh"},"context_window":{"used_percentage":10,"total_input_tokens":40000,"context_window_size":400000}}' | \
  GICC_SESSION_MODE=ultracode GICC_STATUSLINE_COLUMNS=200 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
...
nonpersistent_status=$(printf '%s\n' '{"session_id":"stable-session","model":{"id":"gpt-5.6-sol"},"context_window":{"used_percentage":0,"total_input_tokens":0,"context_window_size":400000,"current_usage":null}}' | \
  GICC_NO_SESSION_PERSISTENCE=1 GICC_STATUSLINE_COLUMNS=200 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline")
[[ "$nonpersistent_status" != *'% context'* ]]
printf '%s\n' '{"session_id":"ephemeral-session","model":{"id":"gpt-5.6-sol"},"context_window":{"used_percentage":25}}' | \
  GICC_NO_SESSION_PERSISTENCE=1 CLAUDE_CONFIG_DIR="$tmp/home/.config/gpt-in-claude-code" "$root/statusline" >/dev/null
[[ ! -e "$tmp/home/.config/gpt-in-claude-code/statusline-cache/ephemeral-session" ]]
...
[[ "$(node "$root/bin/gicc-package.mjs" --package-version)" == "$(node -p "require('$root/package.json').version")" ]]
...
install_output=$(HOME="$install_home" PATH="$tmp/bin:$PATH" \
  GICC_PROXY_TOKEN='installer-test-token' \
  GICC_PROXY_CONFIG="$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml" \
  GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 \
  "$root/install.sh")
[[ -x "$install_home/.local/bin/gicc" ]]
[[ -x "$install_home/.config/gpt-in-claude-code/statusline" ]]
[[ -x "$install_home/.config/gpt-in-claude-code/usage-limit" ]]
[[ -x "$install_home/.config/gpt-in-claude-code/codex-session" ]]
[[ -r "$install_home/.config/gpt-in-claude-code/preload.cjs" ]]
[[ -x "$install_home/.config/gpt-in-claude-code/self-update" ]]
[[ -r "$install_home/.config/gpt-in-claude-code/skills/usage-limit/SKILL.md" ]]
[[ -r "$install_home/.config/gpt-in-claude-code/skill-bridge.cjs" ]]
[[ -r "$install_home/.config/gpt-in-claude-code/settings.json" ]]
[[ -r "$install_home/.config/gpt-in-claude-code/env" ]]
[[ -r "$install_home/.config/gpt-in-claude-code/install.json" ]]
[[ "$install_output" != *'installer-test-token'* ]]
printf -v expected_statusline '%q' "$install_home/.config/gpt-in-claude-code/statusline"
jq -e --arg expected "/usr/bin/env bash $expected_statusline" \
  '.statusLine.command == $expected and .tui == "fullscreen"' \
  "$install_home/.config/gpt-in-claude-code/settings.json" >/dev/null
installed_env=$(<"$install_home/.config/gpt-in-claude-code/env")
[[ "$installed_env" == *'GICC_PROXY_TOKEN=installer-test-token'* ]]
[[ "$installed_env" == *'GICC_PROXY_CONFIG='* ]]
[[ "$installed_env" == *'GICC_PROXY_URL=http://127.0.0.1:8318'* ]]
[[ "$installed_env" == *'GICC_CODEX_AUTH_DIR='* ]]
[[ -r "$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml" ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")" == *'host: "127.0.0.1"'* ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")" == *'port: 8318'* ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")" == *'request-retry: 3'* ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")" == *'transient-error-cooldown-seconds: 1'* ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")" == *'bootstrap-retries: 2'* ]]
jq -e --arg version "$(node -p "require('$root/package.json').version")" \
  '.schema == 1 and .version == $version and .method == "git" and .repository == "DrPei12/gpt-in-claude-code"' \
  "$install_home/.config/gpt-in-claude-code/install.json" >/dev/null
...
rollback_env_before=$(<"$install_home/.config/gpt-in-claude-code/env")
rollback_proxy_before=$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")
printf '%s\n' rollback-statusline-sentinel > "$install_home/.config/gpt-in-claude-code/statusline"
if HOME="$install_home" PATH="$tmp/bin:$PATH" GICC_PROXY_TOKEN='must-not-survive' \
  GICC_INSTALL_METHOD=invalid GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 \
...
[[ "$(<"$install_home/.config/gpt-in-claude-code/env")" == "$rollback_env_before" ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")" == "$rollback_proxy_before" ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/statusline")" == rollback-statusline-sentinel ]]
[[ -z "$(find "$install_home/.config/gpt-in-claude-code" -maxdepth 1 -name '.install-transaction.*' -print -quit)" ]]
...
crash_transaction="$install_home/.config/gpt-in-claude-code/.install-transaction.crash-test"
mkdir -p "$crash_transaction/backup"
crash_targets=(
  "$install_home/.local/bin/gicc"
  "$install_home/.config/gpt-in-claude-code/env"
  "$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml"
  "$install_home/.config/gpt-in-claude-code/bin/gicc-proxy"
  "$install_home/.config/gpt-in-claude-code/settings.json"
  "$install_home/.config/gpt-in-claude-code/statusline"
  "$install_home/.config/gpt-in-claude-code/usage-limit"
...
printf '%s\n' 'GICC_PROXY_TOKEN=corrupted-crash-token' > "$install_home/.config/gpt-in-claude-code/env"
recovery_output=$(HOME="$install_home" PATH="$tmp/bin:$PATH" \
  GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 "$root/install.sh")
[[ "$recovery_output" == *'Recovered the previous interrupted GICC installation'* ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/env")" == *'GICC_PROXY_TOKEN=installer-test-token'* ]]
[[ ! -e "$crash_transaction" ]]
...
custom_proxy_config="$tmp/custom-gicc-proxy.yaml"
custom_proxy_bin="$tmp/custom-gicc-proxy"
custom_auth_dir="$tmp/custom-gicc-auth"
awk -v proxy_config="$custom_proxy_config" -v proxy_bin="$custom_proxy_bin" -v auth_dir="$custom_auth_dir" '
  /^GICC_PROXY_URL=/ { print "GICC_PROXY_URL=http://127.0.0.1:9123"; next }
  /^GICC_PROXY_CONFIG=/ { print "GICC_PROXY_CONFIG=" proxy_config; next }
  /^GICC_PROXY_BIN=/ { print "GICC_PROXY_BIN=" proxy_bin; next }
  /^GICC_CODEX_AUTH_DIR=/ { print "GICC_CODEX_AUTH_DIR=" auth_dir; next }
  { print }
' "$install_home/.config/gpt-in-claude-code/env" > "$tmp/installer-custom-env"
mv "$tmp/installer-custom-env" "$install_home/.config/gpt-in-claude-code/env"
HOME="$install_home" PATH="$tmp/bin:$PATH" GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 \
  "$root/install.sh" >/dev/null
custom_installed_env=$(<"$install_home/.config/gpt-in-claude-code/env")
[[ "$custom_installed_env" == *'GICC_PROXY_URL=http://127.0.0.1:9123'* ]]
[[ "$custom_installed_env" == *"GICC_PROXY_CONFIG=$custom_proxy_config"* ]]
[[ "$custom_installed_env" == *"GICC_PROXY_BIN=$custom_proxy_bin"* ]]
[[ "$custom_installed_env" == *"GICC_CODEX_AUTH_DIR=$custom_auth_dir"* ]]
[[ "$(<"$install_home/.config/gpt-in-claude-code/cliproxyapi.yaml")" == *"auth-dir: \"$custom_auth_dir\""* ]]
invocation_proxy_config="$tmp/invocation-gicc-proxy.yaml"
invocation_proxy_bin="$tmp/invocation-gicc-proxy"
invocation_auth_dir="$tmp/invocation-gicc-auth"
HOME="$install_home" PATH="$tmp/bin:$PATH" \
  GICC_PROXY_TOKEN='invocation-test-token' GICC_PROXY_URL='http://127.0.0.1:9234' \
  GICC_PROXY_CONFIG="$invocation_proxy_config" GICC_PROXY_BIN="$invocation_proxy_bin" \
  GICC_CODEX_AUTH_DIR="$invocation_auth_dir" GICC_SKIP_DEPENDENCY_INSTALL=1 \
  GICC_SKIP_SERVICE_START=1 "$root/install.sh" >/dev/null
invocation_installed_env=$(<"$install_home/.config/gpt-in-claude-code/env")
[[ "$invocation_installed_env" == *'GICC_PROXY_TOKEN=invocation-test-token'* ]]
[[ "$invocation_installed_env" == *'GICC_PROXY_URL=http://127.0.0.1:9234'* ]]
[[ "$invocation_installed_env" == *"GICC_PROXY_CONFIG=$invocation_proxy_config"* ]]
[[ "$invocation_installed_env" == *"GICC_PROXY_BIN=$invocation_proxy_bin"* ]]
[[ "$invocation_installed_env" == *"GICC_CODEX_AUTH_DIR=$invocation_auth_dir"* ]]
...
HOME="$install_home" PATH="$tmp/bin:$PATH" GICC_PROXY_PORT=9345 \
  GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 "$root/install.sh" >/dev/null
port_migrated_env=$(<"$install_home/.config/gpt-in-claude-code/env")
[[ "$port_migrated_env" == *'GICC_PROXY_URL=http://127.0.0.1:9345'* ]]
HOME="$install_home" PATH="$tmp/bin:$PATH" GICC_PROXY_PORT=9456 GICC_PROXY_URL='https://proxy.example.test' \
  GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 "$root/install.sh" >/dev/null
custom_url_env=$(<"$install_home/.config/gpt-in-claude-code/env")
[[ "$custom_url_env" == *'GICC_PROXY_URL=https://proxy.example.test'* ]]
if HOME="$install_home" PATH="$tmp/bin:$PATH" GICC_PROXY_PORT='invalid-port' \
  GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 "$root/install.sh" >/dev/null 2>&1; then
...
package_setup_output=$(HOME="$package_home" PATH="$tmp/bin:$PATH" \
  GICC_INSTALL_METHOD=homebrew GICC_PROXY_TOKEN='package-test-token' GICC_SKIP_DEPENDENCY_INSTALL=1 \
  GICC_SKIP_SERVICE_START=1 GICC_SKIP_AUTO_UPDATE=1 node "$root/bin/gicc-package.mjs" --package-setup)
[[ "$package_setup_output" != *'Add this directory to PATH:'* ]]
jq -e --arg version "$(node -p "require('$root/package.json').version")" \
  '.package == "gpt-in-claude-code" and .version == $version and .method == "homebrew"' \
  "$package_home/.config/gpt-in-claude-code/package-manager.json" >/dev/null
[[ -x "$package_home/.config/gpt-in-claude-code/package-bin/gicc" ]]
rm -f "$package_home/.config/gpt-in-claude-code/preload.cjs"
HOME="$package_home" PATH="$tmp/bin:$PATH" \
  GICC_INSTALL_METHOD=homebrew GICC_PROXY_TOKEN='package-test-token' GICC_SKIP_DEPENDENCY_INSTALL=1 \
  GICC_SKIP_SERVICE_START=1 GICC_SKIP_AUTO_UPDATE=1 node "$root/bin/gicc-package.mjs" --version >/dev/null
[[ -r "$package_home/.config/gpt-in-claude-code/preload.cjs" ]]
...
ln -s "$root/bin/gicc-package.mjs" "$package_conflict_bin/gicc"
HOME="$package_conflict_home" PATH="$tmp/bin:$PATH" GICC_BIN_DIR="$package_conflict_bin" \
  GICC_INSTALL_METHOD=homebrew GICC_PROXY_TOKEN='package-conflict-token' GICC_SKIP_DEPENDENCY_INSTALL=1 \
  GICC_SKIP_SERVICE_START=1 GICC_SKIP_AUTO_UPDATE=1 node "$root/bin/gicc-package.mjs" --package-setup >/dev/null
[[ "$(readlink "$package_conflict_bin/gicc")" == "$root/bin/gicc-package.mjs" ]]
[[ -x "$package_conflict_home/.config/gpt-in-claude-code/package-bin/gicc" ]]
...
mkdir -p "$codex_install_home/.config/gpt-in-claude-code/bin" "$codex_install_home/.codex" "$codex_install_bin"
...
HOME="$codex_install_home" PATH="$codex_install_bin:/usr/bin:/bin" \
  FAKE_CODEX_LOGIN_LOG="$codex_login_log" GICC_TEST_INTERACTIVE_INSTALL=1 \
  GICC_SKIP_SERVICE_START=1 "$root/install.sh" >/dev/null
[[ -x "$codex_install_home/.local/bin/codex" ]]
[[ "$(wc -l < "$codex_login_log" | tr -d ' ')" == 1 ]]
[[ ! -e "$codex_install_home/.config/gpt-in-claude-code/run/install.lock" ]]
...
# no compatible Node runtime instead of rolling back on every automatic update attempt.
node_migration_home="$tmp/node migration home"
node_migration_bin="$tmp/node-migration-bin"
mkdir -p "$node_migration_home/.config/gpt-in-claude-code" "$node_migration_home/.codex" "$node_migration_bin"
...
cat > "$node_migration_bin/node" <<'EOF'
#!/usr/bin/env bash
exit 1
EOF
...
EOF
chmod +x "$node_migration_bin/node" "$node_migration_bin/brew"
cat > "$node_migration_home/.config/gpt-in-claude-code/install.json" <<EOF
{"schema":1,"version":"1.4.4","method":"archive","binDir":"$node_migration_home/.local/bin","repository":"DrPei12/gpt-in-claude-code"}
EOF
...
HOME="$node_migration_home" PATH="$node_migration_bin:/usr/bin:/bin" \
  GICC_INSTALL_METHOD=archive GICC_PROXY_TOKEN='node-migration-token' \
  GICC_SKIP_DEPENDENCY_INSTALL=1 GICC_SKIP_SERVICE_START=1 \
...
[[ -r "$node_migration_home/.config/gpt-in-claude-code/skill-bridge.cjs" ]]
...
bootstrap_root="$bootstrap_fixture/gicc-$bootstrap_version"
...
for required in gicc codex-session settings.json; do : > "$bootstrap_root/$required"; done
cat > "$bootstrap_root/install.sh" <<'EOF'
#!/usr/bin/env bash
printf '%s\n' "${GICC_INSTALL_METHOD:-}:$*" > "$FAKE_BOOTSTRAP_INSTALL_LOG"
EOF
chmod +x "$bootstrap_root/install.sh"
(cd "$bootstrap_fixture" && tar -czf "gicc-$bootstrap_version.tar.gz" "gicc-$bootstrap_version")
bootstrap_archive="$bootstrap_fixture/gicc-$bootstrap_version.tar.gz"
bootstrap_digest=$(shasum -a 256 "$bootstrap_archive" | awk '{print $1}')
printf '%s  %s\n' "$bootstrap_digest" "gicc-$bootstrap_version.tar.gz" > "$bootstrap_fixture/SHA256SUMS"
...
if [[ "$output" == /dev/null ]]; then printf '%s' 'https://github.com/DrPei12/gpt-in-claude-code/releases/tag/v9.8.7'; exit; fi
case "$url" in */SHA256SUMS) cp "$FAKE_BOOTSTRAP_FIXTURE/SHA256SUMS" "$output" ;; *) cp "$FAKE_BOOTSTRAP_FIXTURE/gicc-9.8.7.tar.gz" "$output" ;; esac
EOF
...
# A Codex Desktop/CLI account change is picked up while GICC is still open.
mkdir -p "$tmp/home/.config/gpt-in-claude-code/usage-cache/refresh.lock"
printf '%s\n' old > "$tmp/home/.config/gpt-in-claude-code/usage-cache/limits.json"
printf '%s\n' codex-test.json > "$tmp/home/.config/gpt-in-claude-code/codex-usage-account"
sleep 10 & auth_watch_parent=$!
auth_watch_ready="$tmp/auth-watch-ready"
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CODEX_AUTH_DIR="$tmp/home/.cli-proxy-api" \
  GICC_AUTH_WATCH_SECONDS=1 GICC_AUTH_WATCH_READY_FILE="$auth_watch_ready" \
  "$tmp/home/.config/gpt-in-claude-code/codex-session" watch "$auth_watch_parent" &
auth_watcher=$!
...
for _ in {1..200}; do
  if jq -e '.account_id == "account-switched" and .access_token == "codex-switched-access"' \
      "$tmp/home/.cli-proxy-api/codex-gicc-managed.json" >/dev/null 2>&1; then break; fi
  sleep 0.05
done
jq -e '.account_id == "account-switched" and .access_token == "codex-switched-access"' \
  "$tmp/home/.cli-proxy-api/codex-gicc-managed.json" >/dev/null
[[ ! -e "$tmp/home/.config/gpt-in-claude-code/codex-usage-account" ]]
[[ ! -e "$tmp/home/.config/gpt-in-claude-code/usage-cache/limits.json" ]]
...
EOF
HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CODEX_AUTH_DIR="$tmp/home/.cli-proxy-api" \
  "$tmp/home/.config/gpt-in-claude-code/codex-session" sync

if HOME="$tmp/home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=1 FAKE_CODEX_LOGGED_OUT=1 \
  "$root/gicc" --sol test-prompt >"$tmp/logged-out.stdout" 2>"$tmp/logged-out.stderr"; then
  printf '%s\n' 'expected logged-out Codex session to fail' >&2
  exit 1
fi
grep -F 'Codex is logged out. Run `codex login` (or `gicc --login`) and retry.' "$tmp/logged-out.stderr" >/dev/null

bridge_file="$tmp/home/.cli-proxy-api/codex-gicc-managed.json"
cat > "$bridge_file" <<'EOF'
...
if HOME="$tmp/home" PATH="$tmp/bin:$PATH" FAKE_CODEX_LOGOUT_EXIT=9 \
  "$root/gicc" --logout >/dev/null 2>&1; then
...
foreign_resume_output=$(FAKE_CLAUDE_RESUME=1 FAKE_FOREIGN_RESUME=1 GICC_TEST_TTY_OUTPUT=1 run_wrapper)
[[ "$foreign_resume_output" == *'gicc --resume 123e4567-e89b-12d3-a456-426614174000'* ]]
[[ "$foreign_resume_output" != *'gicc --resume 223e4567-e89b-12d3-a456-426614174000'* ]]
ambiguous_resume_output=$(FAKE_CLAUDE_RESUME=1 FAKE_SAME_CWD_RESUME=1 GICC_TEST_TTY_OUTPUT=1 run_wrapper)
[[ "$ambiguous_resume_output" != *'Resume this session with GICC:'* ]]
[[ "$ambiguous_resume_output" != *'gicc --resume 323e4567-e89b-12d3-a456-426614174000'* ]]

direct_resume_output=$(FAKE_CLAUDE_RESUME=1 GICC_TEST_TTY_OUTPUT=1 run_wrapper --claude-chrome)
[[ "$direct_resume_output" == *'gicc --claude-chrome --resume 123e4567-e89b-12d3-a456-426614174000'* ]]

update_home="$tmp/update-home"
mkdir -p "$update_home/.config/gpt-in-claude-code" "$update_home/.codex" "$update_home/.cli-proxy-api"
cp "$root/settings.json" "$update_home/.config/gpt-in-claude-code/settings.json"
cp "$root/codex-session" "$update_home/.config/gpt-in-claude-code/codex-session"
cp "$root/preload.cjs" "$update_home/.config/gpt-in-claude-code/preload.cjs"
cp "$root/skill-bridge.cjs" "$update_home/.config/gpt-in-claude-code/skill-bridge.cjs"
chmod +x "$update_home/.config/gpt-in-claude-code/codex-session"
cp "$tmp/home/.codex/auth.json" "$update_home/.codex/auth.json"
printf '%s\n' 'GICC_PROXY_TOKEN=test-token' "GICC_CODEX_AUTH_DIR=$update_home/.cli-proxy-api" > "$update_home/.config/gpt-in-claude-code/env"
update_dir="$update_home/.config/gpt-in-claude-code/update"
direct_update_log="$tmp/direct-update.log"
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=0 \
  GICC_TEST_MODE=1 GICC_TEST_FORCE_HARDLINK_FAILURE=1 \
  FAKE_UPDATE_LOG="$direct_update_log" "$root/gicc" --claude-chrome --version >/dev/null
...
state_lock="$update_home/.config/gpt-in-claude-code/run/model-display.lock"
mkdir -p "$state_lock"
printf 'pid=%s\nidentity=%s\nnonce=live-state-owner\n' "$$" 'test-process-identity' > "$state_lock/owner"
touch -t 200001010000 "$state_lock"
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=1 \
  GICC_TEST_MODE=1 GICC_TEST_PROCESS_IDENTITY=test-process-identity "$root/gicc" --claude-chrome --version >/dev/null
grep -F 'nonce=live-state-owner' "$state_lock/owner" >/dev/null
rm -rf "$state_lock"
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=1 \
  GICC_TEST_MODE=1 GICC_TEST_FORCE_PUBLICATION_FAILURE=1 \
  "$root/gicc" --claude-chrome --version >/dev/null
...
touch -t 200001010000 "$state_lock"
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=1 \
  GICC_TEST_PS_FAIL=1 "$root/gicc" --claude-chrome --version >/dev/null
...
aba_base=(HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" \
  GICC_SKIP_AUTO_UPDATE=1 GICC_TEST_MODE=1 GICC_TEST_PROCESS_IDENTITY=test-process-identity \
  GICC_TEST_LOCK_MATCH=model-display.lock)
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/aba-a-mkdir" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/aba-a-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
aba_a=$!
for _ in {1..200}; do [[ -e "$tmp/aba-a-mkdir" ]] && break; sleep 0.02; done
touch -t 200001010000 "$state_lock"
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/aba-b-publish" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/aba-b-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
aba_b=$!
...
printf 'pid=99999999\nidentity=dead\nnonce=x\n' > "$state_lock/owner"
touch -t 200001010000 "$state_lock"
env "${aba_base[@]}" GICC_TEST_LOCK_BEFORE_RENAME_READY="$tmp/aba-x-before" \
  GICC_TEST_LOCK_BEFORE_RENAME_CONTINUE="$tmp/aba-x-before-continue" \
  GICC_TEST_LOCK_AFTER_RENAME_READY="$tmp/aba-x-after" \
  GICC_TEST_LOCK_AFTER_RENAME_CONTINUE="$tmp/aba-x-after-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
aba_x=$!
for _ in {1..200}; do [[ -e "$tmp/aba-x-before" ]] && break; sleep 0.02; done
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/aba-y-publish" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/aba-y-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
aba_y=$!
...
env "${aba_base[@]}" "$root/gicc" --claude-chrome --version >/dev/null &
aba_z=$!
wait "$aba_z"
...
printf 'pid=99999999\nidentity=dead\nnonce=x-self\n' > "$state_lock/owner"
touch -t 200001010000 "$state_lock"
env "${aba_base[@]}" GICC_TEST_LOCK_BEFORE_RENAME_READY="$tmp/self-x-before" \
  GICC_TEST_LOCK_BEFORE_RENAME_CONTINUE="$tmp/self-x-before-continue" \
  GICC_TEST_LOCK_AFTER_RENAME_READY="$tmp/self-x-after" \
  GICC_TEST_LOCK_AFTER_RENAME_CONTINUE="$tmp/self-x-after-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
self_x=$!
for _ in {1..200}; do [[ -e "$tmp/self-x-before" ]] && break; sleep 0.02; done
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/self-y-publish" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/self-y-continue" \
  GICC_TEST_LOCK_SELF_RECOVERED_FILE="$tmp/self-y-recovered" \
  "$root/gicc" --claude-chrome --version >/dev/null &
self_y=$!
...
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/legacy-a-mkdir" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/legacy-a-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
legacy_a=$!
...
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/legacy-zero-mkdir" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/legacy-zero-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
legacy_zero=$!
...
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/legacy-absent-mkdir" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/legacy-absent-continue" \
  GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/legacy-absent-entered" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/legacy-absent-after-continue" \
  GICC_TEST_LOCK_PRESERVE_FILE="$tmp/legacy-absent-path-moved" \
  "$root/gicc" --claude-chrome --version >/dev/null &
legacy_absent=$!
...
env "${aba_base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/unknown-owner-mkdir" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/unknown-owner-continue" \
  "$root/gicc" --claude-chrome --version >/dev/null &
unknown_owner=$!
...
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=0 \
  GICC_TEST_MODE=1 GICC_TEST_UPDATE_WORKER_ATTEMPT_FILE="$live_update_attempt" \
  GICC_PROXY_TOKEN=must-not-leak ANTHROPIC_AUTH_TOKEN=must-not-leak GICC_MANAGED_SESSION=1 \
  ...
  FAKE_UPDATE_DONE_FILE="$live_update_done" "$root/gicc" --claude-chrome --version >/dev/null
...
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=0 \
  GICC_TEST_MODE=1 GICC_TEST_UPDATE_WORKER_ATTEMPT_FILE="$live_update_attempt" \
  FAKE_UPDATE_LOG="$live_update_log" "$root/gicc" --claude-chrome --version >/dev/null
...
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=0 \
  ...
  FAKE_UPDATE_WAIT_FILE="$dead_update_release" "$root/gicc" --claude-chrome --version >/dev/null
...
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=0 \
  FAKE_UPDATE_LOG="$dead_update_log" "$root/gicc" --claude-chrome --version >/dev/null
...
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=0 FAKE_UPDATE_LOG="$update_log" \
  FAKE_CLAUDE_DELAY=0.2 "$root/gicc" test-prompt >/dev/null
...
HOME="$update_home" PATH="$tmp/bin:$PATH" GICC_CURL_BIN="$tmp/bin/curl" GICC_SKIP_AUTO_UPDATE=0 \
  GICC_TEST_MODE=1 GICC_TEST_UPDATE_WORKER_ATTEMPT_FILE="$legacy_update_attempt" \
  FAKE_UPDATE_LOG="$update_log" "$root/gicc" --claude-chrome --version >/dev/null

...
GICC installs a small launcher and private configuration around Codex and
...
22 LTS runtime under the private GICC config directory. The Windows installer uses built in PowerShell
...
The shortest verified installation uses the repository bootstrap.
...
1. Download the platform release archive from the
   [latest release](https://github.com/DrPei12/gpt-in-claude-code/releases/latest) and
...
2. Clone the complete repository for development, tests, or contributions:
   ...
   ```bash
   git clone https://github.com/DrPei12/gpt-in-claude-code.git
   cd gpt-in-claude-code
   ```
...
`gicc --fable --print "Reply with OK"`. A native Claude model and a managed
...
GICC checks its stable release channel once per day without blocking startup
and applies updates by default. Inspect, check immediately, or apply immediately
with:

```text
gicc self-update --status
gicc self-update --check
gicc self-update --apply
```

Release archive and source installs use an exact stable GitHub release asset
only after its version, checksum, and archive paths pass validation. A failed
or offline update keeps the installed release intact and retries later with
backoff. See the [package channel status](package-managers.md).
...
`~/.config/gpt-in-claude-code/backups`.
...
GICC, and run the platform installer. Do not copy `auth.json`,
`~/.config/gpt-in-claude-code`, generated proxy keys, history, or session files between
machines. Re authentication is safer and keeps each installation independent.

## Remove GICC

GICC does not modify the normal Claude Code profile. To remove it, first
close active GICC sessions. Back up any intentional custom settings, then
remove the GICC launcher and its private config directory:

```bash
rm "$HOME/.local/bin/gicc"
rm -rf "$HOME/.config/gpt-in-claude-code"
```

Windows PowerShell:

```powershell
Remove-Item "$env:USERPROFILE\.local\bin\gicc.cmd" -Force -ErrorAction SilentlyContinue
Remove-Item "$env:USERPROFILE\.local\bin\gicc.ps1" -Force -ErrorAction SilentlyContinue
Remove-Item "$env:USERPROFILE\.config\gpt-in-claude-code" -Recurse -Force
```

Removing GICC does not uninstall Codex or Claude Code and does not delete

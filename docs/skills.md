...
GICC automatically makes existing Claude Code and Codex skills available in
ordinary GPT backed sessions. Source skills are never moved, rewritten, or
deleted. GICC creates a private compatibility view under
`~/.config/gpt-in-claude-code/skill-bridge` for each project and passes that view to
Claude Code with its native skill discovery interface.

## Discovered locations

| Ecosystem | Locations available inside GICC |
| --- | --- |
...
Set `GICC_CLAUDE_CONFIG_DIR` when the normal Claude profile is not
`~/.claude`. Set `GICC_SKILL_EXTRA_DIRS` to an OS path list of additional
Agent Skills directories.

## Codex instructions

GICC also snapshots the Codex instruction chain into the compatibility
...
GICC accumulates guidance in Codex order, from global through repository root
...
`gicc skills`. Set `GICC_INSTRUCTION_BRIDGE=off` to disable only this
instruction snapshot.

## Invocation and compatibility

GICC publishes each valid standalone bridged skill for Claude Code's native
...
and `context: fork` when those features are present. GICC does not emulate
...
`GICC_SKILL_DOLLAR_REFERENCES=off` to disable only that hook. Run
`gicc skills` outside a session to list the exact aliases, sources,
...
names match, GICC also publishes the natural unqualified form. For example,
`/frontend-design` is a shortcut for
`/frontend-design:frontend-design`. If a native or standalone skill already
owns that unqualified name, GICC omits the shortcut and keeps only the safe
...
GICC applies only the compatibility adaptations that are necessary:
...
- Claude skill model pins using Opus, Fable, Best, Sonnet, or Haiku family IDs,
  ...
  source exact; GICC's Claude family runtime aliases still route their
...
personal profile skills. A native project or GICC managed isolated profile
...
not have at runtime. Plugin namespaces remain separate. `gicc skills` shows
...
Existing plugin packages are never loaded wholesale: GICC copies only their
...
`gicc skills` reports MCP, app, hook, agent, language server, and settings
...
credentials. Use `gicc codex` or `gicc claude` when the workflow needs
...
Claude profile; bare and safe modes intentionally suppress GICC additions.

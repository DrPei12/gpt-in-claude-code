...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/gicc-auth-usage.XXXXXX")
trap 'rm -rf "$tmp"' EXIT
...
wait_for_session_temporary() {
  ...
  while :; do
    find "$GICC_CODEX_AUTH_DIR" -maxdepth 1 -name '.codex-session.tmp.*' -print -quit | grep . >/dev/null && return 0
    (( SECONDS < deadline )) || {
...
export GICC_CONFIG_DIR="$HOME/.config/gpt-in-claude-code"
export CODEX_HOME="$HOME/.codex"
export GICC_CODEX_AUTH_DIR="$GICC_CONFIG_DIR/codex-accounts"
export PATH="$tmp/bin:$PATH"
mkdir -p "$tmp/bin" "$CODEX_HOME" "$GICC_CODEX_AUTH_DIR" "$GICC_CONFIG_DIR/usage-cache"
...
printf '%s\n' '{"type":"codex","access_token":"access-a","refresh_token":"refresh-a","account_id":"account-a"}' \
  > "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json"
printf '%s\n' old > "$GICC_CONFIG_DIR/usage-cache/summary"
printf '%s\n' 1 > "$GICC_CONFIG_DIR/usage-cache/last-success"
printf '%s\n' codex-a.json > "$GICC_CONFIG_DIR/codex-usage-account"
"$root/codex-session" sync
jq -e '.account_id == "account-b" and .id_token == "" and .last_refresh == ""' \
  "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" >/dev/null
[[ ! -e "$GICC_CONFIG_DIR/usage-cache/summary" ]]
[[ ! -e "$GICC_CONFIG_DIR/codex-usage-account" ]]

# GICC deliberately owns a file-backed Codex session even when the user's
...
jq 'del(.email)' "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" > "$tmp/old-projection.json"
mv "$tmp/old-projection.json" "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json"
"$root/codex-session" sync
jq -e '.email == "managed@example.com"' "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" >/dev/null
...
[[ "$(<"$GICC_CONFIG_DIR/codex-usage-account")" == codex-gicc-managed.json ]]
...
session_sync_lock="$GICC_CODEX_AUTH_DIR/.codex-session-sync.lock"
...
wait "$stale_sync_pid"
jq -e '.access_token == "access-current"' "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" >/dev/null
...
mkdir -p "$GICC_CONFIG_DIR/usage-cache"
printf '%s\n' preserved > "$GICC_CONFIG_DIR/usage-cache/summary"
printf '%s\n' codex-gicc-managed.json > "$GICC_CONFIG_DIR/codex-usage-account"
...
GICC_TEST_MODE=1 GICC_TEST_SESSION_SYNC_LOCK_WAIT_READY_FILE="$invalid_lock_ready" \
...
jq -e '.access_token == "access-revalidated"' "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" >/dev/null
...
GICC_TEST_MODE=1 GICC_TEST_SESSION_SYNC_LOCK_WAIT_READY_FILE="$logout_lock_ready" \
...
[[ -e "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" ]]
[[ -e "$GICC_CONFIG_DIR/usage-cache/summary" ]]
[[ -e "$GICC_CONFIG_DIR/codex-usage-account" ]]
rm -rf "$session_sync_lock"
...
rm -rf "$session_sync_lock"
if find "$GICC_CODEX_AUTH_DIR" -maxdepth 1 -name '.codex-session.tmp.*' -print -quit | grep . >/dev/null; then
...
GICC_TEST_MODE=1 GICC_AUTH_WATCH_SECONDS=1 \
  GICC_TEST_AUTH_WATCH_AFTER_INITIAL_SYNC_READY_FILE="$initial_sync_ready" \
  GICC_TEST_AUTH_WATCH_AFTER_INITIAL_SYNC_CONTINUE_FILE="$initial_sync_continue" \
  GICC_AUTH_WATCH_READY_FILE="$tmp/watch-ready" \
...
for _ in {1..200}; do
  jq -e '.account_id == "account-b"' "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" >/dev/null 2>&1 && break
  sleep 0.05
done
jq -e '.account_id == "account-b"' "$GICC_CODEX_AUTH_DIR/codex-gicc-managed.json" >/dev/null
...
[[ "$(<"$GICC_CONFIG_DIR/codex-usage-account")" == codex-c.json ]]
"$root/usage-limit" --account auto >/dev/null
...
export GICC_CURL_BIN="$tmp/bin/fake-curl"
export GICC_USAGE_SOURCE=web
export GICC_USAGE_REFRESH_SECONDS=60
export GICC_USAGE_MAX_STALE_SECONDS=60

usage_url_error="$tmp/usage-url-error"
blocked_curl_arguments="$tmp/blocked-curl-arguments"
if GICC_USAGE_SOURCE=auto \
  GICC_USAGE_URL='http://127.0.0.1:8123/backend-api/wham/usage' \
...
grep -F 'GICC_USAGE_URL must remain https://chatgpt.com/backend-api/wham/usage' "$usage_url_error" >/dev/null
[[ ! -e "$blocked_curl_arguments" ]]

if GICC_INSECURE_TEST_ALLOW_USAGE_URL=1 \
  GICC_USAGE_URL='https://example.com/backend-api/wham/usage' \
  FAKE_CURL_ARGUMENTS_FILE="$blocked_curl_arguments" \
  "$root/usage-limit" --refresh-cache >/dev/null 2>"$usage_url_error"; then
...
GICC_INSECURE_TEST_ALLOW_USAGE_URL=1 GICC_USAGE_URL="$loopback_usage_url" \
...
summary=$(<"$GICC_CONFIG_DIR/usage-cache/summary")
...
mkdir -p "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
...
touch -t 202001010000 "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
"$root/usage-limit" --refresh-cache
[[ ! -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
...
mkdir -p "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
printf '%s\n' "$$ live-token-123" > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid"
printf '%s\n' "$(( $(date +%s) - 121 ))" > "$GICC_CONFIG_DIR/usage-cache/last-attempt"
...
printf '%s\n' '{"session_id":"lock-test","model":{"id":"gpt-5.6-sol"},"context_window":{"used_percentage":1}}' | \
  GICC_TEST_MODE=1 GICC_TEST_USAGE_REFRESH_EXIT_FILE="$statusline_refresh_exit" \
  CLAUDE_CONFIG_DIR="$GICC_CONFIG_DIR" GICC_USAGE_LIMIT_BIN="$root/usage-limit" "$root/statusline" >/dev/null
[[ -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
[[ "$(( $(date +%s) - $(<"$GICC_CONFIG_DIR/usage-cache/last-attempt") ))" -le 2 ]]
wait_for_file "$statusline_refresh_exit" 'detached statusline usage refresh completion' 1
printf '%s\n' '99999999 dead-token-123' > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid"
touch -t 202001010000 "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
"$root/usage-limit" --refresh-cache
[[ ! -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
...
mkdir -p "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
printf '%s\n' "$$ fresh-token-123" > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid"
...
[[ "$(<"$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid")" == "$$ fresh-token-123" ]]
rm -rf "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
...
mkdir "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
printf '%s\n' "$reuse_nonce" > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation"
printf 'pid=%s\nidentity=%s\nnonce=%s\n' "$$" 'deliberately-wrong-identity' "$reuse_nonce" \
  > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid"
touch -t 202001010000 "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
GICC_TEST_MODE=1 GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  "$root/usage-limit" --refresh-cache
[[ ! -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
...
GICC_TEST_MODE=1 GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  GICC_TEST_FORCE_REFRESH_HARDLINK_FAILURE=1 \
  "$root/usage-limit" --refresh-cache
[[ ! -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
...
GICC_TEST_MODE=1 GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  GICC_TEST_REFRESH_LOCK_AFTER_MKDIR_READY_FILE="$ab_ready" \
  GICC_TEST_REFRESH_LOCK_AFTER_MKDIR_CONTINUE_FILE="$ab_continue" \
...
rm -rf "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
mkdir "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
printf '%s\n' 'replacement-b-123' > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation"
...
[[ "$(<"$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation")" == replacement-b-123 ]]
rm -rf "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
...
GICC_TEST_MODE=1 GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  GICC_TEST_REFRESH_LOCK_AFTER_MKDIR_READY_FILE="$empty_b_ready" \
  GICC_TEST_REFRESH_LOCK_AFTER_MKDIR_CONTINUE_FILE="$empty_b_continue" \
...
[[ -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
[[ ! -e "$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation" ]]
[[ ! -e "$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid" ]]
rm -rf "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
...
GICC_TEST_MODE=1 GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  GICC_TEST_REFRESH_LOCK_AFTER_MKDIR_READY_FILE="$old_b_ready" \
  GICC_TEST_REFRESH_LOCK_AFTER_MKDIR_CONTINUE_FILE="$old_b_continue" \
...
rm -rf "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
mkdir "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
printf '%s\n' "$legacy_live_record" > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid"
: > "$old_b_continue"
...
[[ ! -e "$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation" ]]
[[ "$(<"$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid")" == "$legacy_live_record" ]]
...
[[ "$(<"$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid")" == "$legacy_live_record" ]]
mv "$GICC_CONFIG_DIR/usage-cache/refresh.lock" \
  "$GICC_CONFIG_DIR/usage-cache/refresh.lock.quarantine.legacy-live"
printf '%s\n' 'injected-new-generation-123' \
  > "$GICC_CONFIG_DIR/usage-cache/refresh.lock.quarantine.legacy-live/generation"
touch -t 202001010000 "$GICC_CONFIG_DIR/usage-cache/refresh.lock.quarantine.legacy-live"
...
[[ -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
[[ ! -e "$GICC_CONFIG_DIR/usage-cache/refresh.lock.quarantine.legacy-live" ]]
[[ ! -e "$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation" ]]
printf '%s\n' '99999999 legacy-dead-owner-123' > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/owner-pid"
touch -t 202001010000 "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
"$root/usage-limit" --refresh-cache
[[ ! -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
...
GICC_TEST_MODE=1 GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  GICC_TEST_REFRESH_LOCK_AFTER_PUBLISH_READY_FILE="$self_ready" \
  GICC_TEST_REFRESH_LOCK_AFTER_PUBLISH_CONTINUE_FILE="$self_continue" \
...
mv "$GICC_CONFIG_DIR/usage-cache/refresh.lock" \
  "$GICC_CONFIG_DIR/usage-cache/refresh.lock.quarantine.self-test"
: > "$self_continue"
wait "$refresh_self_pid"
[[ ! -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock" ]]
[[ ! -d "$GICC_CONFIG_DIR/usage-cache/refresh.lock.quarantine.self-test" ]]
...
GICC_TEST_MODE=1 \
  GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  GICC_TEST_REFRESH_LOCK_BEFORE_RENAME_READY_FILE="$xy_before_ready" \
  GICC_TEST_REFRESH_LOCK_BEFORE_RENAME_CONTINUE_FILE="$xy_before_continue" \
  GICC_TEST_REFRESH_LOCK_AFTER_RENAME_READY_FILE="$xy_after_ready" \
  GICC_TEST_REFRESH_LOCK_AFTER_RENAME_CONTINUE_FILE="$xy_after_continue" \
...
rm -rf "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
mkdir "$GICC_CONFIG_DIR/usage-cache/refresh.lock"
printf '%s\n' 'replacement-y-123' > "$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation"
...
GICC_TEST_MODE=1 GICC_TEST_REFRESH_PROCESS_IDENTITY="$test_identity" \
  GICC_TEST_REFRESH_LOCK_CONTENDED_READY_FILE="$z_contended_ready" \
...
if [[ -r "$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation" ]]; then
  [[ "$(<"$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation")" == replacement-y-123 ]]
else
  replacement_y_barrier=$(grep -l '^replacement-y-123$' \
    "$GICC_CONFIG_DIR"/usage-cache/refresh.lock.quarantine.*/generation 2>/dev/null | head -1 || true)
...
[[ "$(<"$GICC_CONFIG_DIR/usage-cache/refresh.lock/generation")" == replacement-y-123 ]]
rm -rf "$GICC_CONFIG_DIR/usage-cache/refresh.lock" \
  "$GICC_CONFIG_DIR/usage-cache/refresh.lock.quarantine.$refresh_x_pid".*
...
if FAKE_APP_SERVER_MODE=deadline GICC_USAGE_SOURCE=app-server GICC_USAGE_TIMEOUT_SECONDS=1 \
...
if FAKE_APP_SERVER_MODE=noisy FAKE_APP_SERVER_CHILD_FILE="$child_file" \
    GICC_USAGE_SOURCE=app-server GICC_USAGE_TIMEOUT_SECONDS=1 \
...
status_cache="$GICC_CONFIG_DIR/statusline-cache"
...
printf '%s\n' '{"session_id":"current-session","model":{"id":"gpt-5.6-sol"},"context_window":{"used_percentage":2}}' | \
  GICC_USAGE_DISPLAY=off CLAUDE_CONFIG_DIR="$GICC_CONFIG_DIR" "$root/statusline" >/dev/null
...
"$root/usage-limit" --refresh-cache
old=$(( $(date +%s) - 120 ))
printf '%s\n' "$old" > "$GICC_CONFIG_DIR/usage-cache/last-success"
...
grep -F '[GICC.CappedTextReader]::DrainAsync' "$root/usage-limit.ps1" >/dev/null
grep -F 'taskkill.exe /PID' "$root/usage-limit.ps1" >/dev/null
grep -F 'function Assert-SafeUsageUrl' "$root/usage-limit.ps1" >/dev/null
grep -F 'GICC_INSECURE_TEST_ALLOW_USAGE_URL permits only loopback HTTP(S) usage endpoints.' "$root/usage-limit.ps1" >/dev/null

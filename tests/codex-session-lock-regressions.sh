...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/gicc-session-lock.XXXXXX")
...
lock="$GICC_CODEX_AUTH_DIR/.codex-session-sync.lock"
mkdir -p "$tmp/bin" "$CODEX_HOME" "$GICC_CODEX_AUTH_DIR"
...
base=(GICC_TEST_MODE=1 GICC_TEST_PROCESS_IDENTITY=session-lock-test GICC_TEST_LOCK_MATCH=.codex-session-sync.lock)
write_auth
...
env "${base[@]}" GICC_TEST_FORCE_HARDLINK_FAILURE=1 "$root/codex-session" sync
...
env "${base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/mixed-a-ready" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/mixed-a-continue" \
...
env "${base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/empty-a-ready" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/empty-a-continue" \
...
env "${base[@]}" GICC_TEST_LOCK_AFTER_MKDIR_READY="$tmp/a-ready" \
  GICC_TEST_LOCK_AFTER_MKDIR_CONTINUE="$tmp/a-continue" \
  "$root/codex-session" sync >"$tmp/a.out" 2>"$tmp/a.err" & a=$!
wait_file "$tmp/a-ready"
touch -t 200001010000 "$lock"
env "${base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/b-ready" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/b-continue" \
...
printf 'pid=99999999\nidentity=dead\nnonce=x\n' > "$lock/owner"
touch -t 200001010000 "$lock"
env "${base[@]}" GICC_TEST_LOCK_BEFORE_RENAME_READY="$tmp/x-before" \
  GICC_TEST_LOCK_BEFORE_RENAME_CONTINUE="$tmp/x-before-continue" \
  GICC_TEST_LOCK_AFTER_RENAME_READY="$tmp/x-after" \
  GICC_TEST_LOCK_AFTER_RENAME_CONTINUE="$tmp/x-after-continue" \
  "$root/codex-session" sync >"$tmp/x.out" 2>"$tmp/x.err" & x=$!
wait_file "$tmp/x-before"
env "${base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/y-ready" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/y-continue" \
...
printf 'pid=99999999\nidentity=dead\nnonce=x-self\n' > "$lock/owner"
touch -t 200001010000 "$lock"
env "${base[@]}" GICC_TEST_LOCK_BEFORE_RENAME_READY="$tmp/sx-before" \
  GICC_TEST_LOCK_BEFORE_RENAME_CONTINUE="$tmp/sx-before-continue" \
  GICC_TEST_LOCK_AFTER_RENAME_READY="$tmp/sx-after" \
  GICC_TEST_LOCK_AFTER_RENAME_CONTINUE="$tmp/sx-after-continue" \
  "$root/codex-session" sync >"$tmp/sx.out" 2>"$tmp/sx.err" & sx=$!
wait_file "$tmp/sx-before"
env "${base[@]}" GICC_TEST_LOCK_AFTER_PUBLISH_READY="$tmp/sy-ready" \
  GICC_TEST_LOCK_AFTER_PUBLISH_CONTINUE="$tmp/sy-continue" \
  GICC_TEST_LOCK_SELF_RECOVERED_FILE="$tmp/sy-recovered" \
